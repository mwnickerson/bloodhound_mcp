...
INTEGRATION_ENABLED = os.getenv("BLOODHOUND_INTEGRATION_TESTS", "0").lower() in ("1", "true", "yes")
...
class TestBloodhoundConnectivity:
    ...
    def test_can_connect_to_bloodhound(self):
        ...
        try:
            ...
            print(f"   API version: {version_info.get('API', {}).get('current_version', 'Unknown')}")
    ...
    def test_can_authenticate(self):
        ...
        try:
            ...
            if roles:
                admin_role = next((r for r in roles if 'admin' in r.get('name', '').lower()), None)
                if admin_role:
                    permissions = admin_role.get('permissions', [])
                    graphdb_perms = [p for p in permissions if p.get('authority') == 'graphdb']
            ...
            if 'result' in result:
                ...
                if 'nodes' in cypher_result:
                    ...
                    if isinstance(nodes_data, list):
                        ...
                        print(f"   Found {len(kerberoastable_users)} Kerberoastable users")
        ...
        try:
            ...
            if 'result' in result and 'nodes' in result['result']:
                ...
                if isinstance(nodes_data, list):
                    ...
                    print(f"   Domain admin groups data is not a list: {type(nodes_data)}")
            else:
    ...
    def test_multiple_api_calls(self):
        ...
        try:
            ...
            calls = [
                ...
                ("simple_cypher", lambda: main.run_cypher_query("MATCH (n) RETURN count(n) as total")),
